...
export const signin = async (req, res, next) => {
    ...
    try {
        ...
        if (!validUser) return next(errorHandler(404, 'not found credentials'))
...
//             const expiryDate = new Date(Date.now() + 360000)
//             res.cookie('access_token', token, { httpOnly: true, expires: expiryDate }).res(200).json(rest)
    ...
    try {
        ...
        if (user) {
            ...
            const newUser = new User({
                ...
                //     Math.random(Math.random().toString(36).slice(-8)),
